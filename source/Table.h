...
public:
	...
	//static const int HASH_SIZE = 3; // a prime number
...
private:
	...
	// representative invariable : 0 <= numEnt, 0 <= numNEBucket <= hashSize, 0 < hashSize

	int mapTraverse(bool flag) const;

	unsigned int hashSize; // size of the hash table
	// (used in hashCode method above)
	ListType* data;
	unsigned int numEnt;
	unsigned int numNEBucket;
};
